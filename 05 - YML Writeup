...
# Quick delete and replace command: sudo rm -r docker-compose.yml && sudo nano docker-compose.yml 
...
# directory to where movies are uploaded via RClone
     - /mnt/rclone-d/zilch/movies:/movies
...
# this is just a test directory; not needed
     - /mnt/rclone-d/zilch/test2:/test2
# this is just a test directory; not needed
...
# Changing the ports will not affect or break any other programs
     - 8015:80
   volumes:
