...
rc = call("touch /var/plexguide/pg.preinstall.stored", shell=True)

# Call Variables
with open('/var/plexguide/pg.preinstall', 'r') as myfile:
    starter=myfile.read().replace('\n', '')

with open('/var/plexguide/pg.preinstall.stored', 'r') as myfile:
...
if starter != stored:
    ...
    menu = plexguide-menu("INSTALLING: PG Folders",
    ...
    rc = call("ansible-playbook /opt/plexguide/menu/interface/folders/main.yml", shell=True)

    # If Successful, Make them Equal to Prevent Future Execution!
    rc = call("cat /var/plexguide/pg.preinstall > /var/plexguide/pg.preinstall.stored", shell=True)
