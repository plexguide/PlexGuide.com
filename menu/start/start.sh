...
read -p '↘️  Type Number | Press [ENTER]: ' typed < /dev/tty
  ...
  if [ "$typed" == "3" ]; then
    ...
    if [ "$edition" == "PG Edition - GDrive" ]; then bash /opt/plexguide/menu/pgclone/pgclone.sh
  elif [ "$edition" == "PG Edition - GCE Feed" ]; then bash bash /opt/plexguide/menu/pgclone/pgclone.sh
