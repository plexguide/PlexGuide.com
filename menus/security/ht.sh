...
#
#################################################################################
echo "INFO - @APPGUARD Starting Process Interface" > /var/plexguide/pg.log && bash /opt/plexguide/roles/log.sh
  ...
  if dialog --stdout --title "Username & Password" \
    ...
    echo "INFO - Username & Password for AppGuard - SET" > /var/plexguide/pg.log && bash /opt/plexguide/roles/log.sh

    ### Builds the Username & Password
    htpasswd -cbs /var/plexguide/server.ht $user $pw 1>/dev/null 2>&1
    echo "INFO - Password for AppGuard is Hashed" > /var/plexguide/pg.log && bash /opt/plexguide/roles/log.sh
    ...
    echo "SUCCESS - APPGuard Deployed" > /var/plexguide/pg.log && bash /opt/plexguide/roles/log.sh

    exit
  else
    echo "WARNING - Elected To Not Deploy AppGuard" > /var/plexguide/pg.log && bash /opt/plexguide/roles/log.sh
