...
#
#################################################################################
  dialog --title "SET SONARR PATH [ EXAMPLE: /unionfs/tv ]" \
  --backtitle "Visit https://PlexGuide.com - Automations Made Simple" \
  --inputbox "Full Path: " 8 52 2>/tmp/pgtrak.sonarr
  ...
  if dialog --stdout --title "PG Path Builder" \
    ...
    if [ "$initial" != "/" ]
      then
            ...
            echo "$path" > /tmp/plexguide/pgtrak.sonarr
      fi
    ...
    echo "$path" > /var/plexguide/pgtrak.sonarr
