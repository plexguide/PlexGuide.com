...
echo "SUCCESS - Path $path is valid! Setting CHOWN & CHMOD." > /var/plexguide/pg.log && bash /opt/plexguide/roles/log.sh

        chown 1000:1000 "$path"
        chmod 0775 "$path"
      else
        dialog --title "PG Path Checker" --msgbox "\nPATH: $path\n\nTHE PATH does not EXIST! Re-Running Menu!" 0 0
echo "WARNING - Path $path does not exist!" > /var/plexguide/pg.log && bash /opt/plexguide/roles/log.sh
...
echo "WARNING - Path $path is NOT Correct!" > /var/plexguide/pg.log && bash /opt/plexguide/roles/log.sh
