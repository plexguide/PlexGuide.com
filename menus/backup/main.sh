...
echo 'INFO - @Backup Cron Job Menu' > /var/plexguide/pg.log && bash /opt/plexguide/roles/log.sh
...
TITLE="Schedule a Backup of --$display --?"
    ...
    case $CHOICE in
            A)
                echo 'INFO - MONTHLY CRON JOB for $display Selected' > /var/plexguide/pg.log && bash /opt/plexguide/roles/log.sh
            ...
            B)
                echo 'INFO - WEEKLY CRON JOB for $display Selected' > /var/plexguide/pg.log && bash /opt/plexguide/roles/log.sh
                ...
                --msgbox "\nBackups of -- $display -- will occur!" 0 0 ;;
            C)
                echo 'INFO - DAILYCRON JOB for $display Selected' > /var/plexguide/pg.log && bash /opt/plexguide/roles/log.sh
                ...
                --msgbox "\nBackups of -- $display -- will occur!" 0 0 ;;
            Z)
                echo 'INFO - NO CRON JOB for $display Selected' > /var/plexguide/pg.log && bash /opt/plexguide/roles/log.sh
