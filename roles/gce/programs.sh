...
echo 'INFO - @GCE Program Select Menu' > /var/plexguide/pg.log && bash /opt/plexguide/roles/log.sh
...
case $CHOICE in
    A)
    echo 'INFO - Selected: NZBGet' > /var/plexguide/pg.log && bash /opt/plexguide/roles/log.sh
    ...
    D)
    echo 'INFO - Selected: SABNZBD' > /var/plexguide/pg.log && bash /opt/plexguide/roles/log.sh
    clear && ansible-playbook /opt/plexguide/pg.yml --tags sabnzbd
    read -n 1 -s -r -p "Press any key to continue"
    ;;

    E)
    echo 'INFO - Selected: CloudST2' > /var/plexguide/pg.log && bash /opt/plexguide/roles/log.sh
