...
echo 'INFO - @Main PG Menu - Local HD Edition' > /var/plexguide/pg.log && bash /opt/plexguide/roles/log.sh
...
case $CHOICE in
        A)
echo 'INFO - Selected: PG Programs Interface Menu' > /var/plexguide/pg.log && bash /opt/plexguide/roles/log.sh
            bash /opt/plexguide/roles/programs/main.sh ;;
        B)
        echo 'INFO - Selected: PG Traefik - Reverse Proxy' > /var/plexguide/pg.log && bash /opt/plexguide/roles/log.sh
                    ...
                    ;;
        C)
echo 'INFO - Selected: PG Security Interface Menu' > /var/plexguide/pg.log && bash /opt/plexguide/roles/log.sh
            bash /opt/plexguide/menus/security/main.sh ;;
        D)
            #### Solo Drive Edition
            if [ "$edition" == "PG Edition: HD Solo" ]
              then
              echo 'WARNING - Using Solo HD Edition! Cannot Configure Drive' > /var/plexguide/pg.log && bash /opt/plexguide/roles/log.sh
...
file="/usr/bin/mergerfs"
  if [ -e "$file" ]
    then
echo 'INFO - MergerFS is Already Installed' > /var/plexguide/pg.log && bash /opt/plexguide/roles/log.sh
    else
echo 'INFO - Installing MERGER FS' > /var/plexguide/pg.log && bash /opt/plexguide/roles/log.sh
...
rm mergerfs*_amd64.deb mergerfs*_amd64.changes mergerfs*.dsc mergerfs*.tar.gz #1>/dev/null 2>&1
            ...
            ;;
        E)
echo 'INFO - Selected: PG Server Information Menu' > /var/plexguide/pg.log && bash /opt/plexguide/roles/log.sh
            bash /opt/plexguide/roles/info-tshoot/infodrives.sh
            ;;
        F)
echo 'INFO - Selected: PG Troubleshoot Interface Menu' > /var/plexguide/pg.log && bash /opt/plexguide/roles/log.sh
            bash /opt/plexguide/roles/info-tshoot/tshoot.sh
            ;;
        G)
echo 'INFO - Selected: Settings for Drive(s) Edition' > /var/plexguide/pg.log && bash /opt/plexguide/roles/log.sh
            bash /opt/plexguide/menus/settings/drives.sh
            ;;
        H)
echo 'INFO - Selected: PG Program Upgrade Interface' > /var/plexguide/pg.log && bash /opt/plexguide/roles/log.sh
            bash /opt/plexguide/scripts/upgrade/main.sh
            bash /opt/plexguide/roles/ending/ending.sh
            exit 0 ;;
        Z)
echo 'INFO - Selected: Exit PlexGuide' > /var/plexguide/pg.log && bash /opt/plexguide/roles/log.sh
            bash /opt/plexguide/roles/ending/ending.sh
            exit 0 ;;
esac

## repeat menu when exiting
echo 'INFO - Looping Local Main Interface Menu' > /var/plexguide/pg.log && bash /opt/plexguide/roles/log.sh
bash /opt/plexguide/roles/localmain.sh
