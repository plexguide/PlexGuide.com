...
#
# Execution: bash /opt/plexguide/roles/baseline/scripts/watchtower.sh
...
if [ "$edition" == "PG Edition: GCE Feed" ]; then
    ...
    echo "INFO - $keyword1: GCE Feeder Edition - Skipping $keyword2 Install" > /var/plexguide/pg.log && bash /opt/plexguide/roles/log/log.sh
    exit 0  
fi

if [ "$pg_watchtower" == "$pg_watchtower_stored" ]; then
    echo "95" | dialog --gauge "$keyword2 Is Already Installed" 7 50 0
    echo "INFO - $keyword1: $keyword2 is Already Installed" > /var/plexguide/pg.log && bash /opt/plexguide/roles/log/log.sh
    sleep 2
else
    echo "95" | dialog --gauge "Installing: $keyword2" 7 50 0
    echo "INFO - $keyword1: $keyword2 is Installing" > /var/plexguide/pg.log && bash /opt/plexguide/roles/log/log.sh
    sleep 2
