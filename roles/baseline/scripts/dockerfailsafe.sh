  ...
  if [ -e "$file" ]
    then
      ...
      file="/usr/bin/docker" 1>/dev/null 2>&1
        if [ -e "$file" ]
          then
          echo "INFO - SUCCESS: Docker Failsafe Resulted in a Succesful Docker Install" > /var/plexguide/pg.log && bash /opt/plexguide/scripts/log.sh
        else
            touch /var/plexguide/startup.error 1>/dev/null 2>&1
          echo "INFO - FAILED: Docker Failsafe Resulted in a Failed Docker Install" > /var/plexguide/pg.log && bash /opt/plexguide/scripts/log.sh
            exit
      fi
fi
