...
case $CHOICE in
        A)
            ...
            clear && ansible-playbook /opt/plexguide/pg.yml --tags pgrole --extra-vars "mod=ombi4k"
            read -n 1 -s -r -p "Press any key to continue"

            ;;
        B)
            echo 'INFO - Selected: Sonarr4k' > /var/plexguide/pg.log && bash /opt/plexguide/roles/log/log.sh
            clear && ansible-playbook /opt/plexguide/pg.yml --tags pgrole --extra-vars "mod=sonarr4k"
            read -n 1 -s -r -p "Press any key to continue"

            ;;
        C)
            echo 'INFO - Selected: Radarr4k' > /var/plexguide/pg.log && bash /opt/plexguide/roles/log/log.sh
            clear && ansible-playbook /opt/plexguide/pg.yml --tags pgrole --extra-vars "mod=radarr4k"
            read -n 1 -s -r -p "Press any key to continue"
