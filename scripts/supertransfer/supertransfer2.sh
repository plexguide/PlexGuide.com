...
clean_up() {
  ...
  numSuccess=$(wc -l < /tmp/superTransferUploadSuccess)
  numFail=$(wc -l < /tmp/superTransferUploadFail)
...
init_DB() {
  ...
  wait
  gdsaLeast=$(sort -gr -k2 -t'=' ${gdsaDB} | grep -E "^GDSA[0-9]+=." | tail -1 | cut -f1 -d'=')
...
while true; do
  ...
  while IFS= read -r -u3 -d $'\0' dir; do
    [[ $(find "${dir}" -type f -mmin -${modTime} -print -quit) == '' && ! $(find "${dir}" -name "*.partial~") ]] && a[i++]=$(du -s "${dir}")
