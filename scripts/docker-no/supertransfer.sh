...
detect_throttle() {
  ...
  upspeed=$(
    iftop 2>/dev/null -t -s $sample_size | \
    grep "1e100.net" -B1 | \
    grep '=>' | \
    grep Mb | \
    awk '{print $4}' | \
...
rclone_sync() {
  ...
  *) drive_chunk_size="16M" ;;
  ...
  # function input = gdrive remote name
  rclone move "$local_dir" "$1:$remote_dir" "${rcloneARGS[@]}"
}
...
queued_transfers() {
  find /mnt/move ! -name "*.partial*" -type f -size +100M | wc -l
}

current_transfers() {
  tail -n 20 /opt/appdata/plexguide/rclone | grep -m1 "Transferring:" -A20 | grep "\*" | wc -l
}
