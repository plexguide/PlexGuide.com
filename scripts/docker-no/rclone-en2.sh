...
## Create the UnionFS Service for the plex encrypted metadata mount point
tee "/etc/systemd/system/unionfs-plex.service" > /dev/null <<EOF
[Unit]
Description=UnionFS Daemon
After=multi-user.target
[Service]
...
echo 6a
## Create the Plex Move Script
tee "/opt/appdata/plexguide/move-plex.sh" > /dev/null <<EOF
#!/bin/bash
sleep 30
while true
do
...
User=0
Group=0
ExecStart=/bin/bash /opt/appdata/plexguide/move-plex.sh
TimeoutStopSec=20
KillMode=process
RemainAfterExit=yes
Restart=always
...
systemctl enable move-plex 1>/dev/null 2>&1
systemctl enable unionfs-plex 1>/dev/null 2>&1
...
systemctl start move-plex 1>/dev/null 2>&1
systemctl start unionfs-plex 1>/dev/null 2>&1
