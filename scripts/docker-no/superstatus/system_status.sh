...
while true; do
	...
	[[ -z $maxdisk_bytes ]] && maxdisk_bytes=1
	...
	du -h /mnt/plexdrive | tail -1 | awk '{print $1}' > /tmp/gdrive_size
		...
		if [[ $(grep rutorrent /tmp/applist) ]]; then
			...
			[[ -z $rutorrent_size ]] && rutorrent_size=1
